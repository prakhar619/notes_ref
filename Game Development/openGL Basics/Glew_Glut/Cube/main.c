...
float CubeRotationX = 0, CubeRotationY = 0, CubeRotationZ = 0;
float CubeAngleX,CubeAngleY,CubeAngleZ;
...
void KeyboardFunction(unsigned char key,int X, int Y)
{
  ...
  switch (key)
  {
    case 'D':
    case 'd':
          CubeRotationX += 180.0f * ((float)(Now - LastTime) / CLOCKS_PER_SEC);
          CubeAngleX = DegreesToRadians(CubeRotationX);
    break;
    case 'W':
    case 'w':
          CubeRotationY += 180.0f * ((float)(Now - LastTime) / CLOCKS_PER_SEC);
          CubeAngleY = DegreesToRadians(CubeRotationY);
    break;
    case 'A':
    case 'a':
          CubeRotationZ += 180.0f * ((float)(Now - LastTime) / CLOCKS_PER_SEC);
          CubeAngleZ = DegreesToRadians(CubeRotationZ);
    break;
  }
  LastTime = Now;
}
...
void DrawCube(void)
{
    ...
    ModelMatrix = IDENTITY_MATRIX;
    RotateAboutY(&ModelMatrix, CubeAngleX);
    RotateAboutX(&ModelMatrix, CubeAngleY);
    RotateAboutZ(&ModelMatrix, CubeAngleZ);
